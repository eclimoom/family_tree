...
import { AfterViewInit, Component, ElementRef, HostListener, Input, OnDestroy, Output, ViewChild, EventEmitter, OnChanges, SimpleChanges } from '@angular/core';
import cytoscape, { Core, EventObject, NodeSingular } from 'cytoscape';
import dagre from 'cytoscape-dagre';
import { Genealogy } from '../../../../services/genealogy-store.service';
...
@Component({
  selector: 'app-genealogy-canvas',
  standalone: true,
  imports: [CommonModule],
  templateUrl: './genealogy-canvas.component.html',
  styleUrl: './genealogy-canvas.component.scss',
})
export class GenealogyCanvasComponent implements AfterViewInit, OnDestroy, OnChanges {
  @ViewChild('cy', { static: false }) cyElement!: ElementRef<HTMLDivElement>;

  @Input() currentTree: Genealogy | undefined;
  ...
  ngOnChanges(changes: SimpleChanges) {
    if (changes['currentTree'] && !changes['currentTree'].firstChange) {
      if (this.cy) {
        this.cy.destroy();
        this.cy = null;
      }
      if (this.currentTree && this.cyElement) {
  ...
  private initCytoscape() {
    const container = this.cyElement?.nativeElement;
