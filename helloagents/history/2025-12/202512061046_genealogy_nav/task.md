...
## 8. 测试
- [X] 8.1 为 `GenealogyStoreService` 添加基本单元测试覆盖创建/查询
> 备注: 受限于当前环境 Node 版本低于 Angular CLI 要求，未执行新增单测。
- [X] 8.2 手动回归：路由跳转、Cytoscape 渲染、列表新增显示
> 备注: 构建因 Node 16 不满足 CLI 最低要求而未跑，手动验证待升级 Node 后补充。
